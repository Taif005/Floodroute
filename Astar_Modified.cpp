...
using namespace std;
const int INF=1e9;
struct Edge{int destination;int weight;};
...
void printPath(int n,vector<int>&p){if(p[n]!=-1)
    {printPath(p[n],p);cout<<" -> ";}cout<<locationName[n];
}
void aStar(int V,int s,int d){
    ...
    priority_queue<pair<double,int>,
            ...
            greater<pair<double,int>>> pq;dist[s]=0;pq.push({heuristic(s,d),s});
            int explored=0;while(!pq.empty()){
                int u=pq.top().second;pq.pop();
                if(vis[u])
                continue;
                vis[u]=true;
                explored++;
                if(u==d)break;for(auto e:graph[u])
                {double nd=dist[u]+e.weight;
                    if(nd<dist[e.destination])
                    {dist[e.destination]=nd;par[e.destination]=u;
                ...
                {cout<<"No Safe Path Found\n";
                    ...
                    cout<<"\n";}
int main(){
    ifstream file("astar.txt");
    if(!file){cout<<"Cannot open astar.txt\n";
        return 0;
    }
    int V,E;file>>V>>E;graph.resize(V);
    locationName.resize(V);
    for(int i=0;i<V;i++)file>>locationName[i];
    for(int i=0;i<E;i++){
